...
// Axis and sparse random projection proposals selected by the original RF PAL
// objective. Leaf voting and exact reranking use the existing RF implementation.
class SPORF : public RFClass {
 public:
  struct Options {
    int sample = 200, support = 16, candidates = 14;
    uint32_t seed = 17;
  };

  SPORF(const float *data, int n, int d) : SPORF(data, n, d, Options{}) {}
  SPORF(const float *data, int n, int d, Options options_)
      : RFClass(data, n, d), options(options_) {
    if (options.sample < 2 || options.support < 1 || options.candidates < 4)
      throw std::invalid_argument("Invalid SPORF options");
  }
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int tree = 0; tree < trees; ++tree) {
      ...
      grow_node(rows.begin(), rows.end(), 0, tree, train, knn, scratch);
 ...
 protected:
  ...
  std::vector<Normal> proposals(std::minstd_rand &rng) const {
    std::vector<Normal> pool;
    const int support = std::min(dim, options.support);
    const int axes = options.candidates/2;
    const int randoms = options.candidates-axes;
  ...
  int grow_node(std::vector<int>::iterator begin, std::vector<int>::iterator end, int level, int tree,
    ...
    if (level < depth && n > 1) {
      ...
      auto candidates = proposals(scratch.generator);
    ...
    const int left = grow_node(begin,mid,level+1,tree,train,knn,scratch);
    const int right = grow_node(mid,end,level+1,tree,train,knn,scratch);

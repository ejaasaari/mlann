...
// Sparse PLS proposals fitted to neighbor geometry histograms. The
// original RF PAL objective selects splits; leaf voting and reranking are shared.
class PLSGeomOnly : public RFClass {
 public:
  struct Options {
    int sample = 200, support = 16, candidates = 14, sketch_dim = 64;
    uint32_t seed = 17;
    float identity_weight = 0.25f; // Retains the geometry scaling of the original sketch.
  };

  PLSGeomOnly(const float *data, int n, int d) : PLSGeomOnly(data, n, d, Options{}) {}
  PLSGeomOnly(const float *data, int n, int d, Options options_)
    ...
    if (options.sample < 2 || options.support < 1 || options.candidates < 4 ||
        ...
        !(options.identity_weight >= 0 && options.identity_weight <= 1))
      throw std::invalid_argument("Invalid PLSGeomOnly options");
  }
...
#pragma omp parallel for schedule(dynamic, 1)
 ...
 protected:
  ...
  struct PartitionNode { Normal normal; };
  RowMatrix make_targets(const Eigen::Ref<const UIntRowMatrix> &knn) const {
    const int r = options.sketch_dim, half = r / 2;
    std::minstd_rand rng(options.seed + 271828U);
    // Keep the zero identity block to preserve the original proposal dimensions.
    RowMatrix embedding = RowMatrix::Zero(n_corpus, r);
    // Three independent projection trees; 4/16/64-cell histograms in each.
    const float scale = std::sqrt((1-options.identity_weight) / 9.f);
    for (int partition = 0; partition < 3; ++partition) {
      std::vector<PartitionNode> nodes(63);
      auto sample = sample_unique(n_corpus, std::min(n_corpus, 8192), rng);
      std::function<void(int,int,int,int)> fit = [&](int node, int level, int begin, int end) {
        if (level == 6) return;
        auto &normal = nodes[node].normal;
        normal = random_normal(8, rng);
        const int mid = (begin + end) / 2;
        if (begin < end) {
          std::nth_element(sample.begin()+begin, sample.begin()+mid, sample.begin()+end,
            [&](int a, int c) { return normal.project(corpus.row(a).data()) < normal.project(corpus.row(c).data()); });
          normal.threshold = normal.project(corpus.row(sample[mid]).data());
        }
        fit(2*node+1, level+1, begin, mid); fit(2*node+2, level+1, mid, end);
      };
      fit(0,0,0,sample.size());
#pragma omp parallel for
      for (int i = 0; i < n_corpus; ++i) {
        int node = 0;
        for (int level = 1; level <= 6; ++level) {
          const auto &normal = nodes[node].normal;
          node = 2*node + (normal.project(corpus.row(i).data()) <= normal.threshold ? 1 : 2);
          if (level % 2 == 0) {
            const uint64_t h = hash(options.seed + 7919ULL * partition + node);
            embedding(i, half + h % half) += (h >> 63 ? scale : -scale);
          }
        }
      }
    }

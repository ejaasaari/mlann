...
class PCACE : public MLANN {
 public:
  struct Options { int sample = 300; uint64_t seed = 17; };
  PCACE(const float *data, int n, int d)
      : PCACE(data, n, d, Options{}) {}
  PCACE(const float *data, int n, int d, Options options)
...
#pragma omp for schedule(static)
    ...
    const pca_ce_detail::PALTables tables(std::min<int>(options_.sample, train.rows()), knn.cols());
#pragma omp parallel
    {
...
#pragma omp for schedule(dynamic, 1)
      for (int t = 0; t < trees; ++t) {
        ...
        scratch.generator.seed(uint32_t(pca_ce_detail::mix(options_.seed ^ pca_ce_detail::mix(t))));
 ...
 private:
  ...
  struct Scratch {
    ...
    pca_ce_detail::ThresholdScratch threshold;
  ...
  int grow_node(It begin, It end, int level, int tree,
                ...
                const pca_ce_detail::PALTables &tables,
                Scratch &scratch) {
    using namespace pca_ce_detail;
    ...
    const Eigen::VectorXf direction = pca(a);

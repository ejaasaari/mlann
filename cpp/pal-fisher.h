...
inline Eigen::VectorXf fisher(const Sample &a, float alpha) {
  if (disjoint_labels(a)) return pca(a);
  // Implicit QR did not clear the measured build-time gate for Fisher.
  QueryBasis q(a.x, false);
  const Matrix &x = q.get(a.x);
  const Eigen::MatrixXf sigma = (x.transpose() * x) / a.n();
  if (!(sigma.trace() > 0)) return Eigen::VectorXf::Zero(a.x.cols());
  const Eigen::MatrixXf b = between(a, x);
  Eigen::MatrixXf denominator = sigma - b;
  // Keep the original input dimension in the ridge after changing basis.
  denominator.diagonal().array() += alpha * sigma.trace() / a.x.cols();
  // B v = lambda D v becomes (L^-1 B L^-T) u = lambda u,
  // with D = L L^T and v = L^-T u. Transform only the leading vector back.
  Eigen::LLT<Eigen::MatrixXf> metric(denominator);
  if (metric.info() != Eigen::Success)
    throw std::runtime_error("PAL-Fisher Cholesky factorization failed");
  Eigen::MatrixXf whitened = b.selfadjointView<Eigen::Lower>();
  metric.matrixL().solveInPlace<Eigen::OnTheLeft>(whitened);
  metric.matrixU().solveInPlace<Eigen::OnTheRight>(whitened);
  return q.expand(metric.matrixU().solve(leading(whitened)));
}
...
class PALFisher : public MLANN {
 public:
  struct Options { int sample = 300; uint64_t seed = 17; float alpha = 0.1f; };
  PALFisher(const float *data, int n, int d)
      : PALFisher(data, n, d, Options{}) {}
  PALFisher(const float *data, int n, int d, Options options)
      : MLANN(data, n, d), options_(options) {
    if (options.sample < 2) throw std::invalid_argument("sample must be >= 2");
    if ((!std::isfinite(options.alpha) || options.alpha <= 0))
      throw std::invalid_argument("PAL-Fisher alpha must be finite and positive");
  }
  using MLANN::query;
...
#pragma omp for schedule(static)
    ...
    const pal_fisher_detail::PALTables tables(std::min<int>(options_.sample, train.rows()), knn.cols());
#pragma omp parallel
    {
...
#pragma omp for schedule(dynamic, 1)
      for (int t = 0; t < trees; ++t) {
        ...
        scratch.generator.seed(uint32_t(pal_fisher_detail::mix(options_.seed ^ pal_fisher_detail::mix(t))));
 ...
 private:
  ...
  struct Scratch {
    ...
    pal_fisher_detail::ThresholdScratch threshold;
  ...
  int grow_node(It begin, It end, int level, int tree,
                ...
                const pal_fisher_detail::PALTables &tables,
                Scratch &scratch) {
    using namespace pal_fisher_detail;
    ...
    const Eigen::VectorXf direction = fisher(a, options_.alpha);

...
// Inline definitions for MLANN's retained-membership calibration and forest views.

struct MLANN::ViewCacheInfo {
    int depth, trees;
    size_t bytes, trees_built;
};

struct MLANN::Calibration {
    int trees, depth;
    float threshold;
    double recall;
};

struct MLANN::CostEstimate {
    double votes = 0, candidates = 0, seconds = 0;
    size_t bytes_upper_bound = 0;
    double touched = 0, elected_votes = 0;
};

struct MLANN::FrontierConfiguration {
    Calibration configuration{0, 0, 0, 0};
    CostEstimate cost;
};

struct MLANN::QueryKernelCosts {
    ...
    double rerank_seconds(double candidates) const {
        ...
        const auto lo = hi - 1;
        const double weight = std::log(candidates / lo->first) / std::log(hi->first / lo->first);
    ...
    double leaf_seconds(
        ...
        if (!leaf_by_trees.empty()) {
            auto hi = std::lower_bound(
                leaf_by_trees.begin(), leaf_by_trees.end(), trees, [](const auto& point, int n) {
                    return point.trees < n;
                }
            );
            if (hi == leaf_by_trees.end())
                ...
                const auto lo = hi - 1;
                const double weight =
                    std::log(double(trees) / lo->trees) / std::log(double(hi->trees) / lo->trees);
...
inline void MLANN::enable_tuning(bool structure_only) {
    if (!empty())
...
inline void MLANN::enable_view_cache() {
    check_view(1, 1);
    std::lock_guard<std::mutex> lock(view_cache_mutex);
    cache_views = true;
}

inline void MLANN::clear_view_cache() const {
    std::lock_guard<std::mutex> lock(view_cache_mutex);
...
inline MLANN::ViewCacheInfo MLANN::view_cache_info() const {
    std::lock_guard<std::mutex> lock(view_cache_mutex);
...
inline std::unique_ptr<MLANN> MLANN::make_view(int, int) const {
    throw std::invalid_argument("Autotuning supports KD, RP, PCA, RF and CRAFTML only");
}

inline std::vector<MLANN::Calibration> MLANN::calibrate(
    ...
    std::vector<std::vector<float>> scores(depth - min_depth + 1, std::vector<float>(count, 0.f));
...
inline std::vector<MLANN::FrontierConfiguration> MLANN::calibrate_frontier(
    const Eigen::Ref<const RowMatrix>& queries,
    const Eigen::Ref<const UIntRowMatrix>& truth,
    int min_depth,
    ...
    for (int q = 0; q < queries.rows(); ++q)
        offsets[q + 1] = offsets[q] + truth.cols() + (q < cost_queries ? cost_sample.size() : 0);
    ...
    std::vector<uint32_t> candidate_support(track_support ? cost_queries * cost_sample.size() : 0);
    ...
    stream_requested_scores(queries, ids, offsets, min_depth, [&](int tree, const float* weights) {
        const float divisor = probability_scores() ? float(tree + 1) : 1.f;
        for (int d = min_depth; d <= depth; ++d) {
                ...
                if (q < cost_queries)
                    for (size_t j = 0; j < cost_sample.size(); ++j) {
                        ...
                        if (track_support) {
                            ...
                            candidate_support[size_t(q) * cost_sample.size() + j] = support[pos];
                        }
                    }
            }
...
inline std::vector<MLANN::CostEstimate> MLANN::estimate_costs(
    const Eigen::Ref<const RowMatrix>& queries,
    ...
    if (queries.rows() == 0 || queries.cols() != dim || !queries.allFinite() || sample.empty() ||
        k < 1 || k > n_corpus)
    ...
    auto result = estimate_sampled_work(
        queries, ids, configs, min_depth, probability_scores() && !sparse_tuning_votes(), population
    );
...
inline std::vector<double> MLANN::predict_recall(
    const Eigen::Ref<const RowMatrix>& queries,
    const Eigen::Ref<const UIntRowMatrix>& truth,
    int trees,
    int d,
...
inline bool MLANN::probability_scores() const {
    return false;
}

inline float MLANN::tuning_node_score(uint32_t count, float scale, int) const {
    return float(count) * scale;
}

inline void MLANN::tuning_path(const float*, int, int*) const {
    throw std::logic_error("Unsupported tuning forest");
}

inline void MLANN::prepare_tuning(
    ...
    tuning_intervals.resize(n_trees, std::vector<std::pair<int, int>>(2 * n_leaves - 1, {-1, -1}));
    tuning_begins.resize(n_trees);
}

inline void MLANN::record_tuning_node(
    int tree,
    int node,
...
inline void MLANN::finish_tuning_tree(int tree, const std::vector<int>& rows) {
    if (retain_membership)
        tuning_permutations[tree] = rows;
}

inline void MLANN::check_view(int trees, int d) const {
...
inline bool MLANN::lower_cost(const CostEstimate& a, const CostEstimate& b) {
    return a.seconds < b.seconds ||
           (a.seconds == b.seconds && a.bytes_upper_bound < b.bytes_upper_bound);
}

inline void MLANN::update_recall_frontier(
    ...
    cost.touched =
        scale * std::count_if(candidates.begin(), candidates.end(), [](float s) { return s > 0; });
...
inline std::vector<MLANN::FrontierConfiguration> MLANN::compact_recall_frontier(
...
inline double MLANN::elapsed_seconds(std::chrono::steady_clock::time_point started) {
    return std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
}

inline size_t MLANN::tuning_storage_bound(const Calibration& config, size_t method_bytes) const {
...
inline std::vector<MLANN::CostEstimate> MLANN::estimate_sampled_work(
    const Eigen::Ref<const RowMatrix>& queries,
...
inline bool MLANN::tuning_compact_votes() const {
    return false;
}

inline bool MLANN::sparse_tuning_votes() const {
    return false;
}

inline MLANN::LeafWorkSample MLANN::sample_leaf_work(
    const Eigen::Ref<const RowMatrix>& queries
) const {
...
inline double MLANN::benchmark_leaf_probe(
...
inline Eigen::Vector3d MLANN::solve_nonnegative_leaf_costs(
...
inline std::vector<MLANN::LeafCostProbe> MLANN::prepare_leaf_probes(
    const LeafWorkSample& sample,
    int prefix,
...
inline std::vector<MLANN::LeafCostObservation> MLANN::measure_leaf_probes(
    const LeafWorkSample& sample,
        ...
        for (int threshold_index = 0; threshold_index < 4; ++threshold_index) {
            for (size_t i = 0; i < probes.size(); ++i) {
                ...
                const double seconds = benchmark_leaf_probe(
                    sample, probe.work, threshold, totals.data(), elected, candidates, setup_seconds
                );
...
inline bool MLANN::use_prefix_leaf_costs(Distance dist) const {
...
inline void MLANN::calibrate_leaf_costs(
    const Eigen::Ref<const RowMatrix>& queries,
    QueryKernelCosts& costs,
    Distance dist
) const {
...
inline std::vector<uint32_t> MLANN::small_ip_candidate_pool(Distance dist) const {
...
inline std::vector<uint32_t> MLANN::bounded_candidate_pool() const {
...
inline std::vector<uint32_t> MLANN::sample_cost_ids(
    const std::vector<uint32_t>& sample,
    Distance dist,
    size_t& population
) const {
...
inline void MLANN::calibrate_rerank_costs(
    const Eigen::Ref<const RowMatrix>& queries,
    int k,
    Distance dist,
    bool sorted,
...
inline void MLANN::route_for_timing(const float* query, int* leaves) const {
...
inline double MLANN::benchmark_routing(const Eigen::Ref<const RowMatrix>& queries) const {
...
inline void MLANN::benchmark_vote_updates(
...
inline MLANN::QueryKernelCosts MLANN::benchmark_query_kernels(
    const Eigen::Ref<const RowMatrix>& queries,
    int k,
    Distance dist
) const {
    ...
    for (int repeat = 0; repeat < 5; ++repeat) {
        ...
        before = std::chrono::steady_clock::now();
        exact_knn(
            Eigen::Map<const Eigen::RowVectorXf>(queries.row(repeat % queries.rows()).data(), dim),
            k,
...
inline MLANN::TuningPostings MLANN::make_tuning_postings(
    const Eigen::Ref<const UIntRowMatrix>& ids
) const {
...
inline std::vector<std::vector<int>> MLANN::tuning_posting_positions(
    int tree,
...
inline float MLANN::tuning_node_scale(int tree, int node, TuningMassCache& cache) const {
    if (!probability_scores())
        return 1.f;
    const auto interval = tuning_intervals[tree][node];
...
inline void MLANN::score_tuning_tree(
    const Eigen::Ref<const RowMatrix>& queries,
    const Eigen::Ref<const UIntRowMatrix>& ids,
    ...
    for (int q = 0; q < queries.rows(); ++q) {
        ...
        for (int d = min_depth; d <= max_depth; ++d) {
            ...
            for (size_t offset = offsets[q]; offset < offsets[q + 1]; ++offset) {
                ...
                if (count >= b)
                    scores[depth_offset + offset] =
                        tuning_node_score(uint32_t(count), scale, interval.second - interval.first);
            }
        }
    }
}

inline int MLANN::tuning_score_batch_size(int tree_count, size_t scores_per_tree) {
    int workers = 1;
#ifdef _OPENMP
    ...
    return int(std::max(
        size_t(1),
        std::min(size_t(std::min(workers, tree_count)), budget / (scores_per_tree * sizeof(float)))
    ));
}

template <class Consumer>
inline void MLANN::stream_tuning_scores(
    const Eigen::Ref<const RowMatrix>& queries,
    const Eigen::Ref<const UIntRowMatrix>& ids,
    int min_depth,
    Consumer consume,
...
template <class Consumer>
inline void MLANN::stream_requested_scores(
    const Eigen::Ref<const RowMatrix>& queries,
    const Eigen::Ref<const UIntRowMatrix>& ids,
    const std::vector<size_t>& offsets,
    int min_depth,
...
inline std::pair<int, int> MLANN::tuning_children(int tree, int node) const {
...
inline size_t MLANN::tuning_leaf_slots(int, int d) const {
    return size_t(1) << d;
}

inline int MLANN::tuning_leaf_slot(int node, int level, int d, int) const {
    return (1 << (d - level)) * (node + 1) - (1 << d);
}

inline void MLANN::fill_tuning_leaf(
    ...
    const bool compact = !probability_scores() && maximum <= std::numeric_limits<uint16_t>::max();
...
inline void MLANN::fill_tuning_subtree(
    ...
    if (children.first < 0) {
        fill_tuning_leaf(
            payload.leaves[tuning_leaf_slot(node, level, d, cursor++)], tree, node, counts, touched
        );
    } else {
        fill_tuning_subtree(payload, tree, children.first, level + 1, d, cursor, counts, touched);
        fill_tuning_subtree(payload, tree, children.second, level + 1, d, cursor, counts, touched);
    }
}

inline std::shared_ptr<const MLANN::TuningTreePayload> MLANN::make_tuning_payload(
    int tree,
    int d,
    std::vector<uint32_t>& counts,
    std::vector<uint32_t>& touched
...
inline void MLANN::initialize_shared_payloads(MLANN& view, int trees, int d) const {
...
inline void MLANN::initialize_view(MLANN& view, int trees, int d) const {
    check_view(trees, d);
    view.n_trees = trees;
    view.depth = d;
    view.n_leaves = 1 << d;

...
// Sparse PLS proposals fitted to projections of the training queries. The
// original RF PAL objective selects splits; leaf voting and reranking are shared.
class PLSQuery : public RFClass {
 public:
  struct Options {
  ...
  PLSQuery(const float *data, int n, int d) : PLSQuery(data, n, d, Options{}) {}
  PLSQuery(const float *data, int n, int d, Options options_)
    ...
    if (options.sample < 2 || options.support < 1 || options.candidates < 4 ||
      ...
      throw std::invalid_argument("Invalid PLSQuery options");
  }
  ...
  void grow(int trees, int depth_, const Eigen::Ref<const UIntRowMatrix> &knn,
    ...
    RowMatrix targets = make_targets(knn,train.data());
...
#pragma omp parallel for schedule(dynamic, 1)
 ...
 protected:
  ...
  RowMatrix make_targets(const Eigen::Ref<const UIntRowMatrix> &knn, const float *training) const {
    ...
    RowMatrix targets(knn.rows(),r);
#pragma omp parallel for
    for (int i = 0; i < knn.rows(); ++i)
      for (int j = 0; j < r; ++j) targets(i,j) = projections[j].project(training+size_t(i)*dim);
    return targets;
  }

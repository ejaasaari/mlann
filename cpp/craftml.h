      ...
      for (int t = 0; t < options.n_trees; ++t) {
        try {
          ...
          if (options.feature_dim > 0) {
            ...
            for (int i = 0; i < train.rows(); ++i) {
              ...
              for (int j = 0; j < dim; ++j) {
                ...
                out[hash % options.feature_dim] += (hash >> 63) ? train(i, j) : -train(i, j);
              }
            }
          }
          const Eigen::Map<const RowMatrix, 0, Eigen::OuterStride<>> features(
              ...
              Eigen::OuterStride<>(options.feature_dim > 0 ? projected.outerStride()
                                                           : train.outerStride()));
...
#pragma omp critical(craftml_build_error)
  ...
  std::vector<LabelScore> predict(const float *q) const {
    ...
    for (uint32_t id : scratch.touched) result.push_back({id, scratch.votes[id] / n_trees});
  ...
  void search(const float *q, int k, int budget, float threshold, int *out, Distance dist = L2,
    ...
    if (budget == -1) {
      for (uint32_t id : scratch.touched)
        if (scratch.votes[id] / n_trees > threshold) scratch.candidates.push_back(id);
    } else {
      scratch.ranked.clear();
      for (uint32_t id : scratch.touched)
        scratch.ranked.push_back({id, scratch.votes[id] / n_trees});
      if (scratch.ranked.size() > static_cast<size_t>(budget)) {
        miniselect::pdqselect_branchless(scratch.ranked.begin(), scratch.ranked.begin() + budget,
                                         scratch.ranked.end(), [](const auto &a, const auto &b) {
 ...
 private:
  ...
  struct Scratch {
    ...
    void clear(int corpus_size) {
      if (votes.size() != size_t(corpus_size)) {
        ...
        for (uint32_t id : touched) votes[id] = 0.f;
      }
      touched.clear();
    }
    ...
    void add(const std::vector<LabelScore> &labels) {
      ...
      for (; i < labels.size(); ++i) {
        ...
        if (i + 32 < labels.size()) __builtin_prefetch(votes.data() + labels[i + 32].id, 1, 1);
#endif
        const auto &entry = labels[i];
        if (votes[entry.id] == 0.f) touched.push_back(entry.id);
  ...
  int route(const float *q, const Eigen::MatrixBase<Derived> &centroids) const {
    ...
    for (int c = 0; c < centroids.rows(); ++c) {
      const float score =
          options_.distance == IP ? x.dot(centroids.row(c)) : -(x - centroids.row(c)).squaredNorm();
      if (score > best_score) {
        best_score = score;
        best = c;
      }
  ...
  void accumulate(const float *q, Scratch &scratch, QueryStats *stats) const {
    ...
    if (feature_dim > 0) scratch.projected.resize(routing_batch_size, feature_dim);
    for (int first = 0; first < n_trees; first += routing_batch_size) {
      ...
      for (int t = 0; t < count; ++t) {
        active[t] = t;
        if (feature_dim > 0)
          project(q, dim, scratch.projected.row(t).data(), feature_dim,
  ...
  void make_leaf(Node &node, const std::vector<int> &ids, size_t begin, size_t end,
                 const Eigen::Ref<const UIntRowMatrix> &knn, BuildScratch &scratch) const {
    scratch.touched.clear();
    for (size_t i = begin; i < end; ++i) {
      const uint32_t *labels = knn.row(ids[i]).data();
      for (int j = 0; j < knn.cols(); ++j)
        if (scratch.counts[labels[j]]++ == 0) scratch.touched.push_back(labels[j]);
    }

...
#define MLANN_OTM_REPEAT_16(operation)                                                             \
    ...
    operation(8) operation(9) operation(10) operation(11) operation(12) operation(13)              \
...
struct OneToManyKernel<FallbackOneToMany> {
    ...
    static MLANN_OTM_ALWAYS_INLINE void run(
        const float* query,
        const float* data,
...
struct OneToManyKernel<SveOneToMany> {
  private:
    ...
    static MLANN_OTM_ALWAYS_INLINE float run_one(
        const float* query,
        const float* row,
        const std::size_t dim
    ) {
...
#undef MLANN_OTM_SVE_SUM
            ...
            for (std::size_t j = 0; j < dim; j += svcntw()) {
                const svbool_t predicate = svwhilelt_b32_u64(
                    static_cast<std::uint64_t>(j), static_cast<std::uint64_t>(dim)
                );
                const svfloat32_t query_vector = svld1_f32(predicate, query + j);
                if constexpr (metric == OneToManyMetric::L2) {
...
struct OneToManyKernel<NeonOneToMany> {
  private:
    static MLANN_OTM_ALWAYS_INLINE float32x4_t
    multiply_add(const float32x4_t sum, const float32x4_t left, const float32x4_t right) {
...
struct OneToManyKernel<Avx512OneToMany> {
  private:
    static MLANN_OTM_ALWAYS_INLINE __m512
...
struct OneToManyKernel<Avx2OneToMany> {
  private:
    static MLANN_OTM_ALWAYS_INLINE __m256
    multiply_add(const __m256 sum, const __m256 left, const __m256 right) {
#if defined(__FMA__)
    ...
    static MLANN_OTM_ALWAYS_INLINE float run_one(
        ...
        if constexpr (metric == OneToManyMetric::L2) {
            for (; j + 8 <= dim; j += 8) {
                const __m256 diff =
                    _mm256_sub_ps(_mm256_loadu_ps(query + j), _mm256_loadu_ps(row + j));
                sum = multiply_add(sum, diff, diff);
            }
        } else {
            for (; j + 8 <= dim; j += 8) {
...
MLANN_OTM_ALWAYS_INLINE void compute_one_to_many(
    const float* query,
    const float* data,
    const std::size_t dim,
    const std::uint32_t* indices,
    const std::size_t count,
    const OneToManyMetric metric,
    Output output
) {
    static_assert(
        std::is_assignable<decltype(output[0]), float>::value,
        "one-to-many outputs must accept floating-point scores"
    );
    if (count == 0)
        return;
    if (metric == OneToManyMetric::L2) {
        OneToManyKernel<NativeOneToMany>::template run<OneToManyMetric::L2>(
            query, data, dim, indices, count, output
        );
    } else {
        OneToManyKernel<NativeOneToMany>::template run<OneToManyMetric::IP>(
            query, data, dim, indices, count, output
        );
    }
}

...
class KMeans {
  public:
    ...
    static std::vector<int> assign(
        ...
        // Bound the temporary similarity matrix.
    ...
    static RowMatrix fit(
        ...
        iterations = refine(data, centroids, assignments, counts, spherical, max_iterations);
        return centroids;
    }
    ...
    static int refine(
        ...
        for (int iteration = 0; iteration < max_iterations; ++iteration) {
            auto next = assign(data, centroids, spherical);
            if (next == assignments)
                return iteration;

...
index = mlann.MLANNIndex(data, "PCA")  # one of RP, PCA, RF, or PCACE
...
- `PCA`: PCA tree
- `PCACE`: full-input supervised oblique tree with PAL thresholds
...
This branch adds `PCACE` using the leading query-covariance direction, followed by the hard PAL threshold scan.

```python
index = mlann.MLANNIndex(corpus, "PCACE")
...
The implementation in [cpp/pca-ce.h](cpp/pca-ce.h) uses all input dimensions,
...
C++ callers can supply `PCACE::Options` to the constructor to change the sample
...
OMP_NUM_THREADS=2 python3 -m unittest discover -s tests -p 'test_pca_ce.py'
mkdir -p benchmarks/.build
g++ -std=c++17 -O3 -march=native -fopenmp -DEIGEN_DONT_PARALLELIZE -Icpp/lib tests/test_pca_ce.cpp -o benchmarks/.build/test_method
...
```bash
python3 benchmarks/run_rf_yandex_pareto.py --index PCACE --output benchmarks/pca-ce_results.csv --label PCACE
```

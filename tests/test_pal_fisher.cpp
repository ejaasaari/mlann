...
using Inspect = InspectIndex<PALFisher>;
template <typename Base>
void verify_wide_forest(int dim) {
  ...
  PALFisher::Options o; o.sample=dim == 200 ? 64 : 12;
...
int main() {
  ...
  for (int dim : {7,17,200}) verify_wide_forest<PALFisher>(dim);
  ...
  std::cout << "PALFisher numerical, PAL, full-input, leaf-vote, routing and determinism checks passed\n";
}

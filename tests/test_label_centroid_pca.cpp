...
using Inspect = InspectIndex<LabelCentroidPCA>;
template <typename Base>
void verify_wide_forest(int dim) {
  ...
  LabelCentroidPCA::Options o; o.sample=dim == 200 ? 64 : 12;
...
int main() {
  ...
  for (int dim : {7,17,200}) verify_wide_forest<LabelCentroidPCA>(dim);
  ...
  std::cout << "LabelCentroidPCA numerical, PAL, full-input, leaf-vote, routing and determinism checks passed\n";
}

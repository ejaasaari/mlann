...
void print_usage(const char *program) {
  std::cerr << "Usage: " << program << " <dataset> [options]\n"
            << "Options: --index=RF|PALFisher --trees=N --depth=N --density=F --leaf-votes=N "
...
BenchmarkOptions parse_options(int argc, char **argv) {
  ...
  for (int i = first_option; i < argc; ++i) {
    ...
    if (!(value = option_value(argument, "index")).empty()) {
      if (value != "RF" && value != "PALFisher") throw std::invalid_argument("Unknown index: " + value);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (options.index_type == "PALFisher") storage = std::make_unique<PALFisher>(train.data(), train.rows(), train.cols());
